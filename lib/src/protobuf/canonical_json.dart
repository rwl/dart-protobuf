...
enum JsonEncoding { JSONP, CANONICAL }
...
Map<String, dynamic> _writeToCanonicalJsonMap(_FieldSet fs) {
  ...
  for (var fi in fs._infosSortedByTag) {
    ...
    result['${fi.name}'] = convertToMap(value, fi.type);
  }
...
void _mergeFromCanonicalJsonMap(
  ...
  for (String key in json.keys) {
    ...
    if (fi == null) {
      ...
      fi = registry.getExtension(fs._messageName, fs._meta.tagNumber(key));
...
_convertCanonicalJsonValue(_FieldSet fs, value, int tagNumber, int fieldType,
